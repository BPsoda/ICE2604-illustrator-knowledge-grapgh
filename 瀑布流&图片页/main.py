...
# )
# cursor=conn.cursor()
# cursor.execute("SELECT id,title,tags,userId,createDate,url_foruse FROM illusts")
# a=cursor.fetchall()
# def addjing(x:str):
#     if x in the_tags:
#         return x
...
# with shelve.open("data") as d:
    ...
    # a=d["data"]
    # d["id"]={i[0] for i in a}
    # d["title"]={i[0]:i[1] for i in a}
    # d["userId"]={i[0]:str(i[3]) for i in a}
    # d["createDate"]={i[0]:i[4] for i in a}
    # d["url_foruse"]={i[0]:i[5] for i in a}
    # _t={i[0]:set([addjing(_) for _ in i[2].split(',')]) for i in a}
    # d["tags"]=_t
    # tmp={}
    # for tag in the_tags:
    #     tmp[tag]={i[0] for i in a if tag in _t[i[0]]}
    # d["top_tags"]=tmp
# exit()
with shelve.open("data") as d:
    d_id=d["id"]
...
def mainpage():
    ...
    return render_template("index.html",flask_datas=ret,tags=the_tags,_the_tags=_tags,_the_ftags=_ftags)
@app.route("/illust")
def illust():
